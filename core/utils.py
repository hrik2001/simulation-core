...
def get_or_create_erc20(contract_address: str, chain: Chain):
    try:
        asset = ERC20.objects.get(
            contract_address__iexact=contract_address, chain=chain
        )
    except ERC20.DoesNotExist:
        response = get_erc20_details(
            contract_address, Web3(Web3.HTTPProvider(chain.rpc))
        )
        asset = ERC20(
            chain=chain,
            contract_address=contract_address,
...
def get_or_create_uniswap_lp(contract_address: str, chain: Chain, token_id: str | int):
    try:
        asset = UniswapLPPosition.objects.get(
            contract_address__iexact=contract_address,
            chain=chain,
            token_id=str(token_id),
        )
    except UniswapLPPosition.DoesNotExist:
        w3 = Web3(Web3.HTTPProvider(chain.rpc))
        position_details = get_positions_details(contract_address, w3, int(token_id))
        ...
        asset = UniswapLPPosition(
            ...
            token1=ERC20.objects.get(
                contract_address__iexact=position_details["token1"]
            ),
            token0=ERC20.objects.get(
...
def update_uniswap_lp(asset: UniswapLPPosition):
    ...
    asset.token1 = (
        ERC20.objects.get(contract_address__iexact=position_details["token1"]),
    )
    asset.token0 = (
        ERC20.objects.get(contract_address__iexact=position_details["token0"]),
    )
    asset.save()
    return asset
...
def price_defillama(
    chain_name: str, contract_address: str | list[str], timestamp: int = None
):
...
def price_defillama_multi(
    chain_name: str, contract_addresses: list[str], timestamp: int = None
):
    ...
    for address in contract_addresses:
        try:
            ...
            logging.exception(
                f"Missing data for coin: {data=} {chain_name=} {contract_addresses=}",
                exc_info=True,
            )
            prices[address] = 1
    return prices
