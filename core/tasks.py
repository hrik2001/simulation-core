...
from .dex_quotes.fetch_quotes import kyperswap_job, paraswap_job
...
def task_cryo_logs(
    ...
    try:
        ...
        metadata = CryoLogsMetadata(label=label, chain=chain)
        metadata.save()
    ...
    **kwargs,
        ...
        inputs = [
            {
                ...
                "name": t.strip().split(" ")[-1],
                "indexed": "indexed" in t.strip(),
            }
            for t in types
        ]
        abi = {
    ...
    def write_data_to_parquet(data, label, start_block, end_block):
        ...
        file_path = os.path.join(
            directory,
            f"{CHAIN_NAME}__logs__{label}__{start_block}_to_{end_block}.parquet",
        )
    ...
    while current_block <= end_block:
        ...
        if set(range(current_block, next_block + 1)).intersection(existing_blocks):
            print(
                f"Skipping blocks {current_block} to {next_block} as they already exist."
            )
            current_block = next_block + 1
            continue

        data = fetch_block_data(
            w3, contract_address, current_block, next_block, event_signature
        )
        # if not data.empty:
