...
usdc_lending_pool_address = Web3.to_checksum_address(
    "0x3ec4a293Fb906DD2Cd440c20dECB250DeF141dF1"
)
weth_lending_pool_address = Web3.to_checksum_address(
...
def get_oracle_information(oracle_count: int):
    ...
    def get_oracle_description(oracle_address: str):
        try:
            ...
            print(
                f"Error calling description function for address {oracle_address}: {e}"
            )
            return None
    ...
    def get_oracle_address_from_id(oracle_id: int):
        try:
            contract = web3.eth.contract(
                address=ORACLE_CONTRACT_ADDRESS, abi=ORACLE_INFO_ABI
            )
            oracle_info = contract.functions.oracleInformation(oracle_id).call()
            return oracle_info[2]  # Returning the oracle address
        except Exception as e:
            print(
                f"Error calling oracleInformation function for oracle ID {oracle_id}: {e}"
            )
            return None
...
def get_price_defillama(labels, search_width=4):
    ...
    if len(labels_to_query) > 0:
        ...
        r = requests.get(
            f"https://coins.llama.fi/prices/current/{q}?searchWidth={search_width}h"
        )
        sleep(1)
...
def get_risk_factors(
    ...
    asset_addresses,
    asset_ids,
    contract_address="0xd0690557600eb8Be8391D1d97346e2aab5300d5f",
):
