...
from curve.types import Top5DebtType, ControllerMetadataType, CurveMetricsType, AggregatedSnapshotsType, \
    ...
    CurveDebtCeilingScoresType, SimuliqType, CurveScoresDetailType, AaveUserDataType, CurveUserDataType
...
class Query(graphene.ObjectType):
    ...
    aave_users = graphene.List(AaveUserDataType, start_time=Int(), end_time=Int(), limit=Int(), sort_by=String(),
                               chain=String())
    curve_users = graphene.List(CurveUserDataType, start_time=Int(), end_time=Int(), limit=Int(), sort_by=String(),
                               chain=String())
    ...
    def resolve_curve_users(self, info, start_time=None, end_time=None, limit=None, sort_by=None, chain=None):
        queryset = CurveUserData.objects.all()
        if chain is None:
            chain = "ethereum"
        chain_obj = Chain.objects.get(chain_name__iexact=chain)
        queryset = queryset.filter(chain=chain_obj)
